...
int main(int argc, char *argv[])
{
    ...
    std::vector<double> pose(6, 0.0); // Default values: {0.001, 0, 0, 0, 0, 0.001}
    pose[0] = 0.001;
    pose[5] = 0.001;
    node->declare_parameter<std::vector<double>>("pose", pose);

    // Get the parameter values
    if (!node->get_parameter("pose", pose))
    {
        RCLCPP_ERROR(rclcpp::get_logger("client_test"), "Failed to get 'pose' parameter. Using default values.");
    }

    // Log the pose values for debugging
    RCLCPP_INFO(rclcpp::get_logger("client_test"), "Pose values:");
    for (size_t i = 0; i < pose.size(); ++i)
    {
        RCLCPP_INFO(rclcpp::get_logger("client_test"), "pose[%zu] = %f", i, pose[i]);
    }
